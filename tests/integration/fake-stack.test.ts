...
`,
  ...
  try {
    ...
    const context = parseHerdrPluginContext(
      JSON.stringify({ invocation: { target_pane_id: "pi-pane" } }),
      "deck-pane",
    );
    ...
    client.start();
    await waitFor(
      () => client.connected && client.state?.activity === "working",
    );
    await client.send("abort", {});
    await client.send("setToolExpanded", {
      toolCallId: "call-1",
      expanded: true,
    });
    assert.deepEqual(
      controller.commands.map((command) => command.name),
      ["abort", "setToolExpanded"],
    );
