...
import { CommandExecutionError, PiDeckController } from "../../src/bridge/pi-controller.js";
import type { CommandFrame, CommandName } from "../../src/bridge/protocol.js";
import { createFakePiHarness } from "../helpers.js";

function command<N extends CommandName>(id: string, name: N, args: Extract<CommandFrame, { name: N }>["args"]): CommandFrame {
  return { type: "command", id, name, args } as CommandFrame;
}

async function expectCode(promise: Promise<unknown>, code: string): Promise<void> {
  await assert.rejects(promise, (error: unknown) => error instanceof CommandExecutionError && error.code === code);
}

test("abort and compact enforce idle/working state", async () => {
  const harness = createFakePiHarness();
  const controller = new PiDeckController(harness.pi, harness.context, "pane-1", harness.expansion);
  await expectCode(controller.execute(command("a", "abort", {})), "invalid_state");
  ...
  await expectCode(controller.execute(command("c2", "compact", {})), "invalid_state");
  controller.dispose();
});
...
test("sendUserMessage validates normal, steer, and followUp delivery state", async () => {
  const harness = createFakePiHarness();
  const controller = new PiDeckController(harness.pi, harness.context, "pane-1", harness.expansion);
  await controller.execute(command("n", "sendUserMessage", { message: "normal", delivery: "normal" }));
  await expectCode(controller.execute(command("s0", "sendUserMessage", { message: "steer", delivery: "steer" })), "invalid_state");
  await expectCode(controller.execute(command("f0", "sendUserMessage", { message: "follow", delivery: "followUp" })), "invalid_state");
  harness.setIdle(false);
  await expectCode(controller.execute(command("n2", "sendUserMessage", { message: "normal", delivery: "normal" })), "invalid_state");
  await controller.execute(command("s", "sendUserMessage", { message: "steer", delivery: "steer" }));
  await controller.execute(command("f", "sendUserMessage", { message: "follow", delivery: "followUp" }));
...
test("thinking, model, and active-tool commands validate advertised choices", async () => {
  ...
  await expectCode(controller.execute(command("t2", "setThinkingLevel", { level: "impossible" })), "unknown_thinking_level");
  await controller.execute(command("m", "setModel", { provider: "test", modelId: "model-2" }));
  assert.equal(harness.model.id, "model-2");
  await expectCode(controller.execute(command("m2", "setModel", { provider: "other", modelId: "model-2" })), "unknown_model");
  harness.pi.setModel = async () => false;
  await expectCode(controller.execute(command("m3", "setModel", { provider: "test", modelId: "model-1" })), "model_unavailable");
  await controller.execute(command("tools", "setActiveTools", { tools: ["read", "bash"] }));
  assert.deepEqual(harness.activeTools, ["read", "bash"]);
  await expectCode(controller.execute(command("tools2", "setActiveTools", { tools: ["unknown"] })), "unknown_tool");
  controller.dispose();
});
...
test("per-tool and bulk expansion commands validate tool IDs", async () => {
  const harness = createFakePiHarness();
  const controller = new PiDeckController(harness.pi, harness.context, "pane-1", harness.expansion);
  await controller.execute(command("e", "setToolExpanded", { toolCallId: "call-1", expanded: true }));
  assert.deepEqual(harness.expansion.toolChanges, [{ id: "call-1", expanded: true }]);
  await expectCode(controller.execute(command("e2", "setToolExpanded", { toolCallId: "missing", expanded: true })), "unknown_tool_call");
  await controller.execute(command("g1", "setToolGroupExpanded", { scope: "currentTurn", expanded: false }));
  await controller.execute(command("g2", "setToolGroupExpanded", { scope: "session", expanded: true }));
...
test("model and thinking fallbacks never advertise unscoped registry choices", () => {
  ...
  assert.deepEqual(snapshot.modelChoices.map((choice) => choice.id), ["model-1"]);
  assert.deepEqual(snapshot.allowedThinkingLevels, ["off", "minimal", "low", "medium", "high"]);
  controller.dispose();
});
...
test("thinking-level fallback follows model reasoning metadata", () => {
  ...
  const controller = new PiDeckController(harness.pi, harness.context, "pane-1", harness.expansion);
  assert.deepEqual(controller.snapshot().allowedThinkingLevels, ["low", "medium", "high", "xhigh"]);
...
test("an empty ExtensionContext scopedModels snapshot advertises all available models", () => {
  ...
  const controller = new PiDeckController(harness.pi, harness.context, "pane-1", harness.expansion);
  assert.deepEqual(controller.snapshot().modelChoices.map((choice) => choice.id), ["model-1", "model-2"]);
  controller.dispose();
});
...
test("refreshState returns a complete safe snapshot and state events update turn/tool status", async () => {
  ...
  controller.recordEvent("tool_execution_start", { toolCallId: "call-1", toolName: "read" }, harness.context);
...
test("tool completion recognizes Pi isError events without serializing tool output", () => {
  ...
  controller.recordEvent("tool_execution_start", { toolCallId: "call-1", toolName: "read" }, harness.context);
  controller.recordEvent("tool_execution_end", {
    toolCallId: "call-1",
    toolName: "read",
    isError: true,
...
test("arbitrary Pi and tool errors are sanitized before crossing the control socket", async () => {
  ...
  await assert.rejects(
    controller.execute(command("secret", "sendUserMessage", { message: "hello", delivery: "normal" })),
