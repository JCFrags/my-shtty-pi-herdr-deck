...
test("adopted secret resolution uses the real resolvePaths sibling and ignores sock.secret decoys", async () => { const root = await mkdtemp(join(tmpdir(), "pi-adopted-path-")); const prior = process.env.PI_HERDR_ORCH_RUNTIME_ROOT; process.env.PI_HERDR_ORCH_RUNTIME_ROOT = root; try { const paths = resolvePaths("adopted-test"); await writeFile(paths.secret, "real-secret\n", { mode: 0o600 }); await writeFile(`${paths.socket}.secret`, "decoy-secret\n", { mode: 0o600 }); assert.equal(siblingSecretPath(paths.socket), paths.secret); assert.notEqual(siblingSecretPath(paths.socket), `${paths.socket}.secret`); assert.throws(() => siblingSecretPath("/tmp/not-a-socket"), /PI_SOCKET_PATH_INVALID/); } finally { if (prior === undefined) delete process.env.PI_HERDR_ORCH_RUNTIME_ROOT; else process.env.PI_HERDR_ORCH_RUNTIME_ROOT = prior; } });
