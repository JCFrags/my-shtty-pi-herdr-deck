...
const capableTui = {
  ...
  TUI: CapableTui,
  ProcessTerminal: CapableTerminal,
};
...
test("capability detection accepts component mouse and normalized expansion object", () => {
  ...
  assert.deepEqual(result.expansion?.getStates(), [
    { id: "x", name: "read", expanded: true, status: "complete", turnIndex: 2 },
  ]);
});
...
test("capability detection rejects stock-style TUI without component mouse API", () => {
  ...
  assert.equal(
    hasComponentMouseApi({ TUI: class {}, ProcessTerminal: class {} }),
    false,
  );
  const result = detectPiCapabilities(context, {
    TUI: class {},
...
test("capability detection rejects global-only expansion and missing subscription with one shared message", () => {
  const context = {
    ui: { getToolsExpanded: () => false, setToolsExpanded: () => undefined },
  };
  const result = detectPiCapabilities(context, capableTui);
  assert.equal(result.compatible, false);
