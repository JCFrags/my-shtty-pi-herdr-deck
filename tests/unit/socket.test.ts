...
test("socket location uses XDG_RUNTIME_DIR fallback, sanitized pane ID, and bounded path", () => {
  assert.equal(
    runtimeDirectoryFor(123, { XDG_RUNTIME_DIR: "/run/user/123" }),
    "/run/user/123/pi-herdr-deck-123",
  );
  assert.equal(
    runtimeDirectoryFor(123, {}),
    join(tmpdir(), "pi-herdr-deck-123"),
  );
  const location = socketLocationForPane("workspace 1/pane:2?", {
    runtimeDirectory: "/tmp/pi-deck-test",
  });
  assert.match(location.sanitizedPaneId, /^workspace-1-pane-2-[a-f0-9]{12}$/);
  assert.equal(
    basename(location.socketPath),
    `${location.sanitizedPaneId}.sock`,
  );
...
test("live sockets and non-socket paths are never removed", async () => {
  ...
  await import("node:fs/promises").then(({ writeFile }) =>
    writeFile(regularPath, "do not delete"),
  );
  await assert.rejects(
    recoverStaleSocket(regularPath),
