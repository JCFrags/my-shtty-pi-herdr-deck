...
test("scheduler admits deterministic priority FIFO and enforces parent limit", () => {
  ...
  scheduler.enqueue(task("tsk_b", "normal", 1));
  scheduler.enqueue(task("tsk_a", "high", 2));
  scheduler.enqueue(task("tsk_c", "low", 0, "agt_other"));
  const admissions = scheduler.admitReady();
  assert.deepEqual(
    admissions.map((item) => [item.taskId, item.admitted]),
    [
      ["tsk_a", true],
      ["tsk_b", false],
      ["tsk_c", true],
...
test("scheduler blocks missing and unsuccessful dependencies", () => {
  const scheduler = new DeterministicScheduler();
  scheduler.enqueue({
    ...task("tsk_child", "normal", 1),
    dependencies: [{ taskId: "tsk_parent", requirement: "succeeded" }],
  });
...
test("reuse requires every policy identity to match", () => {
  ...
  assert.equal(
    findReusableAgent(request, [
      { ...request, agentId: "agt_a", idle: true, cleanupPending: false },
    ]),
    "agt_a",
  );
  assert.equal(
    findReusableAgent(request, [
      { ...request, agentId: "agt_a", idle: true, cleanupPending: true },
    ]),
    undefined,
  );
});
test("budget reports unknown dimensions and force stop after grace", () => {
  ...
  assert.equal(assessment.action, "graceful_stop");
  assert.equal(assessment.percent, 100);
  assert.deepEqual(assessment.unavailable, ["costUsd"]);
...
const workflow = {
  ...
  description: "test",
  mode: "dag" as const,
  failureMode: "collect_all" as const,
  ...
  steps: [
    ...
    },
    {
      ...
      profileId: "scout",
      title: "B",
      objectiveTemplate: "Use {{steps.a.summary}}",
      constraints: [],
      dependsOn: ["a"],
      resultProjection: ["/summary"],
      isolationMode: "shared-readonly" as const,
...
test("workflow planner rejects cycles", () => {
  assert.throws(
    () =>
      validateWorkflow({
        ...workflow,
        steps: workflow.steps.map((step) => ({ ...step, dependsOn: ["b"] })),
      }),
    /WORKFLOW_CYCLE/,
  );
});
  ...
  const service = new ParentToolService(
    {
      invoke: async (method) => {
        ...
        return {
          ...
          retrieval: { method: "workflow.get", id: "wfl_1", nextCursor: null },
        };
      },
    },
    new Map([["agt_child", "agt_parent"]]),
    { maxResponseBytes: 10, maxTextBytes: 20 },
  );
  const allowed = await service.execute(
    { tool: "agent_get", input: { agentId: "agt_child" } },
    principal,
  );
  assert.equal(allowed.ok, true);
  assert.equal(calls[0], "agent.get");
  const brokerAuthorized = await service.execute(
    { tool: "agent_get", input: { agentId: "agt_other" } },
    principal,
  );
  assert.equal(brokerAuthorized.ok, true);
});
