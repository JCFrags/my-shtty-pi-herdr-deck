...
test("orchestrator extension reload transfers runtime credential and registers tools once", async () => {
    ...
    socket.on("data", (data) => {
      for (const item of decoder.push(data)) {
        ...
        if (frame.type === "hello")
          ...
          if (frame.method === "agent.register_managed") registrationCount++;
          socket.write(
            encodeFrame({
              v: 1,
              type: "response",
              ...
              result:
                frame.method === "agent.register_managed"
                  ? {
                      agentId: "agt_child",
                      generation: 1,
                      connectionGeneration: registrationCount,
                      heartbeatMs: 5000,
                      permissions: ["read:state"],
                    }
                  : {},
  ...
  try {
    ...
    await waitFor(() => registrationCount === 1);
    const firstShutdown = first.handlers.get("session_shutdown") ?? [];
    for (const handler of firstShutdown)
      await handler({ reason: "reload" }, firstHarness.context);
    ...
    await waitFor(() => registrationCount === 2);
    assert.equal(first.tools.length, 2);
    assert.equal(second.tools.length, 2);
    assert.equal(
      new Set(
        (second.tools as Array<{ name: string }>).map((tool) => tool.name),
      ).size,
      2,
    );
    assert.equal(connections[0]?.destroyed, true);
    const secondShutdown = second.handlers.get("session_shutdown") ?? [];
    for (const handler of secondShutdown)
      await handler({ reason: "quit" }, secondHarness.context);
    assert.equal(
