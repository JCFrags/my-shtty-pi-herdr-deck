...
test("parent tool metadata converges names to broker methods", () => {
  ...
  assert.deepEqual(PARENT_TOOL_METADATA.agent_get.targetParameters, [
    "agentId",
  ]);
  assert.deepEqual(PARENT_TOOL_METADATA.agent_wait.targetParameters, [
    "agentId",
    "taskId",
    "runId",
  ]);
  assert.deepEqual(PARENT_TOOL_METADATA.agent_result.targetParameters, [
    "taskId",
  ]);
  assert.deepEqual(PARENT_TOOL_METADATA.agent_answer.targetParameters, [
    "questionId",
  ]);
  assert.deepEqual(PARENT_TOOL_METADATA.task_collect.targetParameters, [
    "taskIds",
  ]);
...
test("parent tool request validation accepts the frozen request shape", () => {
  const result = validateParentToolRequest({
    tool: "task_get",
    input: { taskId: "tsk_1" },
...
test("parent tool request validation rejects unknown and oversized fields", () => {
  ...
  if (!result.valid)
    assert.deepEqual(
      ...
      ["$.extra", "$.tool", "$.input", "$.idempotencyKey"],
    );
});
