  ...
  const server = createServer((socket) => {
    ...
    socket.write(
      encodeFrame({
        ...
        type: "state",
        seq: 3,
        payload: baseState({ sessionId: "new" }),
      }),
    );
    socket.write(
      encodeFrame({
        ...
        type: "state",
        seq: 2,
        payload: baseState({ sessionId: "stale" }),
      }),
    );
  });
...
test("a second client is explicitly rejected while one controller is attached", async () => {
  ...
  await server.start();
  const first = new BridgeClient({
    socketPath: server.socketPath,
    reconnectDelaysMs: [10],
  });
  const second = new BridgeClient({
    socketPath: server.socketPath,
    reconnectDelaysMs: [10],
  });
  first.start();
  ...
  await waitFor(
    () =>
      second.status.kind === "disconnected" &&
      second.status.reason.includes("already attached"),
  );
...
test("malformed commands receive errors and do not terminate the server", async () => {
  ...
  socket.write(
    encodeFrame({ type: "command", id: "ok", name: "refreshState", args: {} }),
  );
  const after = await readFrames(socket, 2);
  assert.ok(
    after.some(
      (frame) => frame.type === "result" && frame.id === "ok" && frame.ok,
    ),
  );
...
test("client reconnects with bounded backoff and never queues commands while disconnected", async () => {
  ...
  const client = new BridgeClient({
    socketPath: path,
    reconnectDelaysMs: [10, 20, 30, 40],
  });
  client.start();
  await assert.rejects(
    client.send("abort", {}),
    (error: unknown) =>
      error instanceof BridgeDisconnectedError &&
      /not queued/.test(error.message),
  );
  ...
  const raw = createServer((socket) => {
    ...
    socket.write(
      encodeFrame({
        ...
        type: "state",
        seq: 2,
        payload: baseState({ herdrPaneId: "late" }),
      }),
    );
  });
  await new Promise<void>((resolve, reject) => {
    raw.once("error", reject);
    raw.listen(path, resolve);
  });
...
test("compatibility endpoint rejects once with the shared explicit message", async () => {
  ...
  const server = new CompatibilityRejectionServer({
    paneId: "incompatible",
    reason,
    runtimeDirectory: await temporaryRuntime(),
  });
  ...
  await waitFor(
    () =>
      client.status.kind === "disconnected" && client.status.reason === reason,
  );
...
test("an invalid Pi snapshot closes only the client connection and logs a concise diagnostic", async () => {
  ...
  assert.ok(
    diagnostics.some(
      (message) =>
        message === "initial Pi state snapshot failed: invalid snapshot",
    ),
  );
