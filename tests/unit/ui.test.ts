...
import { type HitBox, PressReleaseTracker } from "../../src/deck/components/controls.js";
...
function mouse(type: "press" | "release" | "move", x: number, y: number, button: "left" | "right" = "left"): TuiMouseEvent {
...
test("one left press/release activates, dragging cancels, right-click is unused", () => {
  ...
  let activations = 0;
  const boxes: HitBox[] = [{ id: "button", x: 2, y: 3, width: 8, height: 1, disabled: false, activate: () => { activations += 1; } }];
...
test("disabled controls do not activate", () => {
  const tracker = new PressReleaseTracker();
  let activations = 0;
  const boxes: HitBox[] = [{ id: "disabled", x: 0, y: 0, width: 10, height: 1, disabled: true, activate: () => { activations += 1; } }];
...
test("pane picker has equivalent keyboard and mouse activation", () => {
  const agents = [
    ...
    { terminalId: "t2", paneId: "p2", agent: "pi", name: "two", focused: false },
  ];
...
test("deck renders disconnected controls as disabled and supports keyboard tab switching", () => {
  const client = new BridgeClient({ socketPath: "/tmp/does-not-exist.sock", reconnectDelaysMs: [] });
  const herdr = new HerdrApi({ runner: async () => ({ stdout: "{}", stderr: "", exitCode: 0 }) });
  const app = new DeckApp({ client, herdr, targetPaneId: "p1", requestRender: () => undefined, getHeight: () => 40 });
...
test("deck controls activate through both keyboard focus and component mouse events", async () => {
  ...
  const server = new BridgeServer({ controller, runtimeDirectory, statePushIntervalMs: 5 });
  await server.start();
  const client = new BridgeClient({ socketPath: server.socketPath, reconnectDelaysMs: [10] });
  const herdr = new HerdrApi({ runner: async () => ({ stdout: "{}", stderr: "", exitCode: 0 }) });
  const app = new DeckApp({ client, herdr, targetPaneId: "pane-1", requestRender: () => undefined, getHeight: () => 40 });
  try {
    client.start();
    ...
    app.handleInput("\r");
    await waitFor(() => controller.commands.filter((command) => command.name === "compact").length === 1);
    ...
    await waitFor(() => controller.commands.filter((command) => command.name === "compact").length === 2);
  } finally {
    app.dispose();
    client.stop();
    await server.close();
