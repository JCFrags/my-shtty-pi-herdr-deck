...
import { detectPiCapabilities, PI_COMPATIBILITY_MESSAGE } from "../src/bridge/capabilities.js";
...
const NO_PANE_MESSAGE = "Pi Deck is inactive because HERDR_PANE_ID is not available. Run Pi inside a Herdr pane.";
...
export default async function piHerdrDeckExtension(api: ExtensionAPI): Promise<void> {
  ...
  const runtime: ExtensionRuntime = {
    active: true,
    status: process.env.HERDR_PANE_ID ? "Pi Deck bridge has not started yet." : NO_PANE_MESSAGE,
    server: undefined,
    ...
    async stopBridge(reason: string): Promise<void> {
      ...
      runtime.status = process.env.HERDR_PANE_ID ? `Pi Deck bridge stopped: ${reason}.` : NO_PANE_MESSAGE;
    },
  ...
  const startBridge = async (context: PiContextLike): Promise<void> => {
    ...
    runtime.startPromise = (async () => {
      ...
      try {
        controller = new PiDeckController(pi, context, paneId, capabilities.expansion);
      } catch {
        await reportCompatibility(context);
        return;
      }
  ...
  pi.registerCommand("herdr-deck-status", {
    ...
    handler: async (_args, context) => {
      if (!runtime.active) return;
      if (process.env.HERDR_PANE_ID && !runtime.server && runtime.status !== PI_COMPATIBILITY_MESSAGE) {
        await startBridge(context);
      }
      context.ui.notify?.(runtime.status, runtime.status === PI_COMPATIBILITY_MESSAGE || runtime.status.includes("failed") ? "error" : "info");
    },
  });
  ...
  for (const eventName of stateEvents) {
    pi.on(eventName, (event, context) => {
      if (runtime.active) runtime.controller?.recordEvent(eventName, event, context);
    });
  }
}
