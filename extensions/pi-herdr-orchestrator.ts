...
export default async function piHerdrOrchestrator(api: ExtensionAPI): Promise<void> {
  ...
  let adapter: PiAdapter | undefined; let client: PiBrokerClient | undefined; let heartbeat: NodeJS.Timeout | undefined;
  ...
  const active = process.env.HERDR_ENV === "1" && !!process.env.HERDR_PANE_ID && !!socketPath && !!process.env.PI_HERDR_ORCH_SESSION_KEY;
  ...
  const start = async (next: PiContextLike): Promise<void> => {
    ...
    client = managed && token ? new PiBrokerClient({ socketPath, sessionKey: process.env.PI_HERDR_ORCH_SESSION_KEY, piSessionId: piSessionId(next), agentId, generation, token }) : new PiBrokerClient({ socketPath, sessionKey: process.env.PI_HERDR_ORCH_SESSION_KEY, piSessionId: piSessionId(next), ...(secret ? { secret } : {}) });
    try { await client.connect(); await client.register(adapter.safeState()); heartbeat = setInterval(() => { if (client?.connected && adapter) void client.heartbeat(adapter.safeState()).catch(() => undefined); }, 5_000); heartbeat.unref(); next.ui.setStatus?.("pi-herdr-orchestrator", managed ? "Managed Pi connected" : "Adopted Pi connected"); } catch { next.ui.setStatus?.("pi-herdr-orchestrator", "Broker unavailable; orchestration controls are disabled."); }
  };
