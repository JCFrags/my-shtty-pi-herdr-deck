import { DeterministicScheduler, findReusableAgent as findFrozenReusableAgent, type Admission } from "./scheduler.js";
...
export function planAdmission(
  ...
  return {
    decisions,
    admittedTaskIds: decisions.filter((decision) => decision.admitted).map((decision) => decision.taskId),
    blockedTaskIds: decisions.filter((decision) => !decision.admitted).map((decision) => decision.taskId),
  };
}

/** Return true only when every reuse identity and lifecycle condition matches. */
export function isReusableCandidate(request: ReuseRequest, candidate: ReuseCandidate): boolean {
 ...
 */
export function selectReusableAgent(request: ReuseRequest, candidates: readonly ReuseCandidate[]): string | undefined {
  return findFrozenReusableAgent(request, candidates.filter((candidate) => isReusableCandidate(request, candidate)));
}
