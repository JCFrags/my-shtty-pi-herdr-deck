...
export type HerdrRunner = (argv: readonly string[]) => Promise<HerdrCommandResult>;
...
function minimalHerdrEnvironment(): NodeJS.ProcessEnv {
  ...
  for (const key of ["PATH", "HOME", "USER", "LOGNAME", "LANG", "LC_ALL", "LC_CTYPE", "TERM", "XDG_RUNTIME_DIR", "HERDR_CONFIG", "HERDR_STATE_DIR"]) {
...
export function schemaSupportsMethod(schema: unknown, method: string): boolean {
  ...
  const visit = (value: unknown, depth: number): boolean => {
    if (depth > 32 || value === null || value === undefined || seen.has(value)) return false;
    ...
    seen.add(value);
    if (Array.isArray(value.methods) && value.methods.every((item) => typeof item === "string")) return value.methods.includes(method);
    const methodProperty = isRecord(value.properties) ? value.properties.method : undefined;
    if (isRecord(methodProperty)) {
      if (methodProperty.const === method) return true;
      if (Array.isArray(methodProperty.enum) && methodProperty.enum.includes(method)) return true;
    }
    for (const key of ["oneOf", "anyOf", "allOf", "items", "$defs", "definitions"]) {
...
export function createHerdrRunner(binaryPath: string, timeoutMs = DEFAULT_TIMEOUT_MS): HerdrRunner {
  if (!binaryPath) throw new HerdrApiError("missing_binary", "HERDR_BIN_PATH is not available.");
  return async (argv) => await new Promise<HerdrCommandResult>((resolve, reject) => {
    ...
    const append = (target: Buffer[], chunk: Buffer): void => {
      ...
      if (outputBytes > MAX_OUTPUT_BYTES) {
        child.kill("SIGKILL");
        finish(new HerdrApiError("output_too_large", "Herdr CLI output exceeded 8 MiB."));
        return;
      }
      target.push(chunk);
    };
    ...
    const timer = setTimeout(() => {
      child.kill("SIGKILL");
      finish(new HerdrApiError("timeout", `Herdr CLI command timed out: ${argv.join(" ")}`));
    }, timeoutMs);
    timer.unref?.();
  });
}
...
function parseJsonOutput(result: HerdrCommandResult, command: string): unknown {
  if (result.exitCode !== 0) {
    throw new HerdrApiError("cli_failed", `${command} failed (exit code ${result.exitCode}).`);
  }
  try {
    return JSON.parse(result.stdout);
  } catch {
    throw new HerdrApiError("invalid_json", `${command} did not return valid JSON.`);
  }
}
...
function normalizeAgent(value: unknown): HerdrAgentInfo | undefined {
  ...
  const displayAgent = optionalString(value.display_agent ?? value.displayAgent);
  const title = optionalString(value.title);
  const status = optionalString(value.agent_status ?? value.agentStatus);
  const cwd = optionalString(value.foreground_cwd ?? value.foregroundCwd ?? value.cwd);
...
export class HerdrApi {
  ...
  constructor(options: { binaryPath?: string; runner?: HerdrRunner }) {
    ...
    else this.#run = createHerdrRunner(options.binaryPath ?? process.env.HERDR_BIN_PATH ?? "");
  }
  ...
  supports(method: string): boolean {
    if (this.#schema === undefined) throw new HerdrApiError("schema_not_loaded", "Herdr API schema has not been loaded.");
  ...
  requireMethods(methods: readonly string[]): void {
    ...
    if (missing.length > 0) {
      throw new HerdrApiError("unsupported_api", `Installed Herdr schema is missing required methods: ${missing.join(", ")}.`);
    }
  }

  async listAgents(): Promise<HerdrAgentInfo[]> {
    this.requireMethods(["agent.list"]);
    const result = parseJsonOutput(await this.#run(["agent", "list"]), "herdr agent list");
    const root = isRecord(result) && isRecord(result.result) ? result.result : result;
