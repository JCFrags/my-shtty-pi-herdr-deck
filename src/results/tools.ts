...
export class ManagedChildTools {
  ...
  async orchestratorResult(
    ...
    return this.service.publish({
      ...
      assignmentGeneration: context.run.assignmentGeneration,
      body,
    });
  }
  async orchestratorAsk(
    context: ManagedToolContext,
    body: QuestionBody,
  ): Promise<{
    questionId: string;
    ...
    const question = await this.service.ask({
      runId: context.run.runId,
      taskId: context.run.taskId,
      agentId: context.run.agentId,
      assignmentGeneration: context.run.assignmentGeneration,
    ...
    const answered = await this.service.waitForAnswer(
      question.id,
      body.timeoutMs,
    );
    return {
      questionId: answered.id,
      ...(answered.answer ? { answer: answered.answer } : {}),
...
export class ParentResultTools {
  constructor(private readonly service: ResultService) {}
  getResult(context: ManagedToolContext, resultId: string) {
    const result = this.service.getResult(resultId);
    return result.taskId === context.run.taskId ||
      result.agentId === context.run.agentId
      ? result
      : undefined;
  }
  answer(
    ...
    return this.service.answer(questionId, answer, context.principalId);
  }
}
