...
export function siblingSecretPath(socketPath: string): string {
  if (!socketPath || socketPath.length > 4096 || !socketPath.startsWith("/") || /[\u0000-\u001f\u007f]/u.test(socketPath) || !socketPath.endsWith(".sock")) throw new Error("PI_SOCKET_PATH_INVALID");
...
export async function readManagedTokenFile(path: string): Promise<string> {
  ...
  let value: string;
  try { value = await readPrivateRegular(path); } catch (error) { const code = (error as NodeJS.ErrnoException).code; if (code === "ELOOP" || code === "ENOENT" || code === "ENOTDIR") throw new Error("PI_TOKEN_FILE_INVALID"); throw error; }
  if (Buffer.byteLength(value, "utf8") > MAX_TOKEN_FILE_BYTES || !/^[A-Za-z0-9_-]{32,2048}\n?$/u.test(value) || value.trimEnd().length > MAX_TOKEN_BYTES) throw new Error("PI_TOKEN_FILE_INVALID");
