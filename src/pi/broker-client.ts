...
export interface PiBrokerClientOptions { socketPath: string; sessionKey: string; agentId?: string; generation?: number; piSessionId: string; token?: string; secret?: string; }
export class PiBrokerClient {
  ...
  async connect(): Promise<unknown> {
    ...
    socket.on("data", (data) => { for (const item of decoder.push(data)) { if (!item.ok || typeof item.value !== "object" || item.value === null) continue; const frame = item.value as Record<string, unknown>; if (typeof frame.id !== "string" || frame.type !== "response") continue; const waiter = this.#pending.get(frame.id); if (!waiter) continue; this.#pending.delete(frame.id); if (frame.ok === true) waiter.resolve(frame.result); else waiter.reject(new Error("BROKER_REQUEST_FAILED")); } });
    ...
    this.#connected = true; return hello;
  }
