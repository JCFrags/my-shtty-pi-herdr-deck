...
export class PiAdapter implements PiControl {
  ...
  async deliver(assignment: PiAssignment): Promise<"accepted" | "already_accepted"> { const result = this.correlator.deliver(assignment, this.safeState()); this.correlator.markCustomEntryWritten(); await this.prompt(renderAssignment(assignment)); this.correlator.accept(); return result; }
