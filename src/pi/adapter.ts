...
export class PiAdapter implements PiControl {
  ...
  async handleControl(method: string, params: Record<string, unknown>): Promise<{ ok: true }> {
    ...
    const allowed = new Set([...identity, ...(method === "control.prompt" || method === "control.steer" ? ["message"] : method === "control.set_model" ? ["provider", "modelId"] : method === "control.set_thinking" ? ["level"] : method === "control.set_tools" ? ["tools"] : method === "control.set_tool_expansion" ? ["name", "expanded"] : [])]);
    ...
    if (method === "control.prompt" || method === "control.steer") { if (typeof params.message !== "string" || params.message.length === 0 || params.message.length > 65_536 || /[\u0000-\u001f\u007f]/u.test(params.message)) throw new Error("INVALID_REQUEST"); if (method === "control.prompt") await this.prompt(params.message); else await this.steer(params.message); }
