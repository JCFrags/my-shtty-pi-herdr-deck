...
import type { CommandArgsMap, CommandName, DeckState, DeliveryMode, ToolExpansionState, ToolStatus } from "../bridge/protocol.js";
import type { HerdrApi } from "../herdr/api.js";
import { type HitBox, PressReleaseTracker, renderButton, truncatePlain } from "./components/controls.js";
...
const STATUS_FILTERS: readonly StatusFilter[] = ["all", "pending", "running", "complete", "error", "unknown"];

function isPrintableInput(data: string): boolean {
  return data.length > 0 && !data.includes("\x1b") && [...data].every((character) => character === "\n" || character === "\t" || character.codePointAt(0)! >= 0x20);
}
...
function statusLabel(status: ClientConnectionStatus): string {
  ...
  if (status.kind === "connecting") return `Connecting (attempt ${status.attempt})`;
  return "Disconnected";
}
...
export class DeckApp implements Component {
  ...
  constructor(options: DeckAppOptions) {
    ...
    if (this.#connection.kind === "disconnected") this.#notice = this.#connection.reason;
    this.#unsubscribers.push(options.client.onStatus((status) => {
      this.#connection = status;
      this.#notice = status.kind === "disconnected" ? status.reason : "";
      if (status.kind !== "connected") this.#dropdown = undefined;
      this.#requestRender();
    }));
    this.#unsubscribers.push(options.client.onState((state) => {
      this.#state = state;
      this.#requestRender();
    }));
  }
  ...
  render(width: number): string[] {
    ...
    this.#addControlRow(lines, [
      { id: "tab:overview", label: "Overview", disabled: false, activate: () => this.#selectTab("overview") },
      { id: "tab:tools", label: "Tools", disabled: false, activate: () => this.#selectTab("tools") },
      { id: "focus-pi", label: "Focus Pi", disabled: false, activate: () => void this.#focusPi() },
    ], safeWidth);
    lines.push(truncatePlain(`Connection: ${statusLabel(this.#connection)}`, safeWidth));
    if (this.#notice) lines.push(truncatePlain(`Notice: ${this.#notice}`, safeWidth));
    lines.push("");
    ...
    if (lines.length < height) lines.push(...Array.from({ length: height - lines.length }, () => ""));
  ...
  handleInput(data: string): void {
    if (this.#dropdown) {
      ...
      else if (data === "\r" || data === "\n" || data === " ") this.#activateDropdownChoice();
    ...
    if (this.#focusedId === "message") {
      ...
      else if (data === "\x7f" || data === "\b") this.#message = this.#message.slice(0, -1);
    ...
    else if (data === "\x1b[A" && this.#tab === "tools") this.#toolScroll = Math.max(0, this.#toolScroll - 1);
  ...
  handleMouse(event: TuiMouseEvent): boolean {
    if (event.type === "wheel") {
      ...
      this.#toolScroll = Math.max(0, this.#toolScroll + (event.direction === "down" ? 2 : -2));
      this.#requestRender();
      return true;
    }
  ...
  #renderOverview(lines: string[], width: number): void {
    ...
    const activity = !connected || !state ? "Unavailable" : state.activity === "working" ? "Working" : "Idle";
    ...
    this.#addControlRow(lines, [{
      ...
      disabled: !connected || (state?.allowedThinkingLevels.length ?? 0) === 0,
    ...
    lines.push(truncatePlain(`Context: ${percentText.padStart(4)}  ${contextBar(percent)}`, width));
    ...
    this.#addControlRow(lines, [
      {
        id: "send:normal",
        label: "Send",
        disabled: !connected || state?.activity !== "idle" || this.#message.trim().length === 0,
        activate: () => void this.#sendMessage("normal"),
      },
      {
        id: "send:steer",
        label: "Steer",
        disabled: !connected || state?.activity !== "working" || this.#message.trim().length === 0,
    ...
    if (state?.lastError) lines.push(truncatePlain(`Pi error: ${state.lastError}`, width));
  }

  #renderMessageEditor(lines: string[], width: number, disabled: boolean): void {
    ...
    this.#hitBoxes.push({
      ...
      height,
      disabled,
      activate: () => { this.#focusedId = "message"; },
    });
    this.#focusControls.push({ id: "message", disabled, activate: () => { this.#focusedId = "message"; } });
  }
  ...
  #renderTools(lines: string[], width: number): void {
    ...
    this.#addControlRow(lines, [
      { id: "turn-expand", label: "Turn Expand", disabled: !connected, activate: () => void this.#setGroup("currentTurn", true) },
      { id: "turn-collapse", label: "Turn Collapse", disabled: !connected, activate: () => void this.#setGroup("currentTurn", false) },
    ], width);
    this.#addControlRow(lines, [
      { id: "session-expand", label: "Session Expand", disabled: !connected, activate: () => void this.#setGroup("session", true) },
      { id: "session-collapse", label: "Session Collapse", disabled: !connected, activate: () => void this.#setGroup("session", false) },
    ], width);
    this.#addControlRow(lines, [{
      id: "status-filter",
    ...
    const filtered = tools.filter((tool) => this.#filter === "all" || tool.status === this.#filter);
    ...
    for (const tool of filtered.slice(this.#toolScroll, this.#toolScroll + room)) this.#renderToolRow(lines, width, tool, !connected);
    if (filtered.length === 0) lines.push(connected ? "No matching tool calls." : "Controls disabled while disconnected.");
    this.#renderDropdown(lines, width);
  }

  #renderToolRow(lines: string[], width: number, tool: ToolExpansionState, disabled: boolean): void {
    ...
    const activate = (): void => void this.#command("setToolExpanded", { toolCallId: tool.id, expanded: !tool.expanded });
    this.#hitBoxes.push({ id, x: 0, y, width: 1, height: 1, disabled, activate });
  ...
  #renderDropdown(lines: string[], width: number): void {
    ...
    lines.push("");
    lines.push(`${this.#dropdown[0]!.toUpperCase()}${this.#dropdown.slice(1)} choices:`);
  ...
  #dropdownOptions(): DropdownOption[] {
    ...
    if (this.#dropdown === "model") {
      return (state?.modelChoices ?? []).map((model) => ({
        ...
        activate: () => void this.#command("setModel", { provider: model.provider, modelId: model.id }),
      }));
    }
    ...
    if (this.#dropdown === "status") {
      return STATUS_FILTERS.map((filter) => ({ key: filter, label: filter, activate: () => { this.#filter = filter; this.#toolScroll = 0; } }));
    }
    return [];
  }
  ...
  #addControlRow(lines: string[], controls: RowControl[], width: number): void {
    ...
    for (const control of controls) {
      ...
      const rendered = renderButton(control.label, { disabled: control.disabled, focused });
      line += rendered;
      this.#hitBoxes.push({ id: control.id, x, y, width: rendered.length, height: 1, disabled: control.disabled, activate: control.activate });
      this.#focusControls.push({ id: control.id, disabled: control.disabled, activate: control.activate });
    }
    lines.push(truncatePlain(line, width));
  }

  #normalizeFocus(): void {
    const current = this.#focusControls.find((control) => control.id === this.#focusedId && !control.disabled);
    if (current) return;
    this.#focusedId = this.#focusControls.find((control) => !control.disabled)?.id ?? "";
  }

  #moveFocus(direction: 1 | -1): void {
    const enabled = this.#focusControls.filter((control) => !control.disabled);
    if (enabled.length === 0) return;
    const current = enabled.findIndex((control) => control.id === this.#focusedId);
    const next = current < 0 ? 0 : (current + direction + enabled.length) % enabled.length;
    this.#focusedId = enabled[next]!.id;
  }

  #activateFocused(): void {
    this.#focusControls.find((control) => control.id === this.#focusedId && !control.disabled)?.activate();
  }
    ...
    const selectedKey = kind === "status"
      ...
      : kind === "thinking"
        ...
        : this.#state?.model
          ...
          : undefined;
    this.#dropdownIndex = Math.max(0, options.findIndex((option) => option.key === selectedKey));
    this.#requestRender();
  }
  ...
  async #command<N extends CommandName>(name: N, args: CommandArgsMap[N]): Promise<boolean> {
  ...
  async #setGroup(scope: "currentTurn" | "session", expanded: boolean): Promise<void> {
  ...
  async #toggleActiveTool(tool: string): Promise<void> {
    ...
    const tools = active ? state.activeTools.filter((name) => name !== tool) : [...state.activeTools, tool];
  ...
  async #focusPi(): Promise<void> {
    try {
      ...
      this.#notice = error instanceof Error ? error.message : "Could not focus Pi pane.";
    }
    this.#requestRender();
  }
}
