...
The parent-tool result is a short list under `availableModels`. Each entry contains only the exact `provider`, `modelId`, and `thinkingLevel` needed by `agent_spawn`, its rank, recommendation flag, and current start capacity. Broker-policy exclusions and internal score components, evidence dates, and digests are intentionally omitted from this user-facing list. The TUI shows five entries when collapsed and all returned entries when expanded. The internal broker view keeps the complete ranking evidence for receipts and audits.
