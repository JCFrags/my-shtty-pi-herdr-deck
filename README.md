...
The read-only `agent_model_options` parent tool ranks only exact provider, model, and thinking pairs that Pi reports as available and that the current task profile, placement, and broker allowlist permit. It calls broker method `model.options`. The request requires `profileId`. It can also include `placement`, `modelProfileId`, `projectKey`, and a model-group result `limit` from 1 to 16. The parent tool requests 16 model groups by default. The broker rejects an eligible scope above 256 exact pairs. It ranks the complete bounded pair set before grouping, so a returned model always includes all its available thinking levels.

The broker method and parent tool both return the same short public list under `availableModels`; this keeps direct broker-backed tool routes from exposing the internal ranking object. The parent tool keeps that public object as JSON in its model-facing result and formats only the interactive TUI. Each entry groups one exact `provider` and `modelId` and lists its spawn-ready thinking levels. Every thinking level keeps its exact global rank, recommendation flag, and whole-star ratings from 0 to 5. The shared `thinkingGuide` gives one short use case for each returned level. `overall` projects the confidence-adjusted final score. `taskFit`, `reliability`, `speed`, and `value` project the corresponding task capability, protocol reliability, speed, and effective-cost components. Each parts-per-million value is rounded to the nearest whole star and formatted as `★★★★★ 5/5`; a negative final score is shown as zero. The response contains no raw scores, broker-policy exclusions, other internal score components, evidence dates, or digests. The collapsed TUI shows each model once with compact thinking-level rows. Expanding it adds the four category ratings. Internal ranking, selection, receipts, and audits continue to use the complete private view.
...
Endpoint capacity is separate from model eligibility and quality. Scheduler admission still enforces `maxConcurrentAgents` for every endpoint. The public model list shows free slots only when the mapped endpoint explicitly sets `resourceClass` to `local_compute`; it then shows one model-group capacity line and says `will queue` when no slot is free. Cloud endpoints should use `remote_service`, and unclassified or derived endpoints omit capacity. The broker never infers locality from provider, model, or endpoint names. Capacity cannot change a quality score, rank group, or ranking digest.
